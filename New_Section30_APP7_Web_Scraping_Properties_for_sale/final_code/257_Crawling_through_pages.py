...
##################
# For Crawling it is dependent on change of the URL
# Find some connection when the page changes and re-execute the code on the new URL
##################
...
# GEt PAge numbers dynamically

resp = requests.get(base_url)
soup_page = BeautifulSoup(resp.content, "html.parser")

page_number_span = soup_page.find_all("span", {"class": "PageNumbers"})[0]

page_number_list = [int(page.text) for page in page_number_span.find_all("a", {"class": "Page"})]

page_number_list = [(page - 1) * 10 for page in page_number_list]

print(page_number_list)

url_list = [base_url + "/t=0&s=" + str(page) + ".html" for page in page_number_list]
...
print(df)
df.to_csv("./property_data_257.csv")
