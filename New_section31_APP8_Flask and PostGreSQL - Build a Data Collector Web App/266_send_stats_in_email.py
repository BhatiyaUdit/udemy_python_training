...
def success():
    if request.method == 'POST':
        ...
        if db.session.query(Data).filter(Data.email == email).count() >= 1:
            ...
            db.session.commit()
            average = db.session.query(func.avg(Data.height)).scalar()
            average = round(average, 2)
            print("AVERAGE HEIGHT :::::: ", average)
            count_users = db.session.query(Data.height).count()
            send_email(email, height, average, count_users)
            # Calculate the average after committing the data to the DB
